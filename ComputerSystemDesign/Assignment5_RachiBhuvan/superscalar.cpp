...
void addlabels(const char* label, unsigned short int &no, int pos, int PC) {
  ...
  unsigned long xx = bin_x.to_ulong();
  no += ((bin_x.to_ulong()&0xff)<<pos);
}
...
void decode(unsigned short int contents, int &opcode, short int &A, short int &B, short int &Imm, short int &R1) {
  ...
  switch(opcode) {
    ...
    case LD:
      ...
      A = registers[R1]; B = registers[R2]; Imm = registers[R3];
    ...
    case MUL_I:
      ...
      registerstall[R1] = true;
      A = registers[R1]; B = registers[R2]; Imm = four2sixteen(R3);
      break;
    case JMP:
      A = R1; B = R2;
      break;
    case BEQZ:
      ...
      A = registers[R1]; B = R2; Imm = R3;
      break;
  }
}

void execute(short int &ALUOutput, int &Cond, short int A, short int B, short int Imm, int opcode, int PC) {
  // If memory reference
  short int x;
  switch(opcode) {
    case LD:
...
void exec_superscalar(short int &ALUOutput, int &Cond, int PC, int &opcode, unsigned short int contents, short int &R1, int t,
  short int &A, short int &B, short int &Imm) {
...
void memory(int &PC, unsigned short int &LMD, int NPC, short int ALUOutput, int opcode, int Cond, short int Imm, int t) {
  //PC = NPC;
  switch(opcode) {
    case LD:
...
bool write_back_superscalar(short int ALUOutput, int opcode, short int R1, unsigned short int LMD, int t) {
  switch(opcode) {
    case ADD:
    case SUB:
    case MUL:
...
bool check_dependency(state_vars S, int instruno) {
  int opcode = (S.contents >> 12);                      //decode
  short int R2,R3;
  short int R1 = ((S.contents & 0xf00) >> 8);
  R2 = ((S.contents & 0xf0) >> 4);
  R3 = (S.contents & 0xf);
  ...
  switch(opcode) {
    ...
    case MUL:
    case LD:
      it1 = registerstall2[R1].begin();  
      it2 = registerstall2[R2].begin();
      it3 = registerstall2[R3].begin();
      if((it1 != registerstall2[R1].end() && *it1 < instruno) || 
        (it2 != registerstall2[R2].end() && *it2 < instruno)  || 
        (it3 != registerstall2[R3].end() && *it3 < instruno)) {
        return true;
      }
      return false;
    case SD:
...
void simulate_superscalar() {
  ...
  while (!done) {
    if (available[0]) {
      ...
      buf[0].time = time;
      wbuftime.insert(time);
    }

    // Fetch at most two instructions from the buffer to execute
    state_vars in1, in2;
    in1.PC = -1; in2.PC = -1;
    int t1, t2;
    for (auto val : ibuf) {
      if (!check_dependency(val.second,val.first))
      {
        if (in1.PC == -1) {
          in1 = val.second;
          t1 = val.first;
    ...
    if (in1.PC != -1) {
      ibuf.erase(t1);
      // execute(in1) 
      exec_superscalar(in1.ALUOutput, in1.Cond, in1.PC, in1.opcode, in1.contents, in1.R1, t1,
        in1.A, in1.B, in1.Imm);
      wbuf[in1.time] = in1;
      // cout << "time = " << in1.time << "\n";
    }
    ...
    if (in2.PC != -1) {
      // if (in1 and in2 can be executed in parallel (WAW/WAR))
     {
        ibuf.erase(t2);
        // execute(PC2);
        exec_superscalar(in2.ALUOutput, in2.Cond, in2.PC, in2.opcode, in2.contents, in2.R1, t2,
          in2.A, in2.B, in2.Imm);
        wbuf[in2.time] = in2;
      }
    }

    // Insert fetched instruction into buffer
    ibuf[time] = buf[0];
    /*
    if (available[3])
    ...
    */
    ...
    // Handle the write back buffer and memory operations
    map<int, state_vars>::iterator it = wbuf.begin();
    if (it != wbuf.end()) {
      state_vars ins = it->second;
      int in_time = it->first;
      int next_write_back = *(wbuftime.begin());
      if (it->first == next_write_back) {
        // write back ins
        memory(ins.PC, ins.LMD, ins.NPC, ins.ALUOutput, ins.opcode, ins.Cond, ins.Imm, it->first);
        done = write_back_superscalar(ins.ALUOutput, ins.opcode, ins.R1, ins.LMD, it->first);
      }
      wbuf.erase(it);
      wbuftime.erase(next_write_back);
    }
    ...
    REP(i,0,2) {
      if(stageflush[i]) {
        ...
        REP(j,0,15) {
          ...
          registerstall2[j].clear();
        }
          for (auto it = ibuf.begin(); it != ibuf.end(); ) {
            if (it->first > flushtimer) {
              it = ibuf.erase(it);
            }
            else {
              ++it;
          ...
          for (auto it = wbuf.begin(); it != wbuf.end(); ) {
            if (it->first > flushtimer) {
              it = wbuf.erase(it);
            }
            else {
          ...
          for (auto it = wbuftime.begin(); it != wbuftime.end(); ) {
            if (*it > flushtimer) {
              it = wbuftime.erase(it);
            }
            else {
              ++it;
            }
          }
          // wbuftime.insert(flushtimer);
      }
    }
...
void simulate_pipeline_real() {
  ...
  while (!done) {
    ...
    if (available[2])
      execute(buf[2].ALUOutput, buf[2].Cond, buf[2].A, buf[2].B, buf[2].Imm, buf[2].opcode, buf[2].PC);
    if (available[3])
      memory(buf[3].PC, buf[3].LMD, buf[3].NPC, buf[3].ALUOutput, buf[3].opcode, buf[3].Cond, buf[3].Imm);
    if (available[4])
      done = write_back(buf[4].ALUOutput, buf[4].opcode, buf[4].R1, buf[4].LMD);
...
void simulate_pipeline() {
  ...
  while (!done) {
    ...
    execute(ALUOutput, Cond,  A, B, Imm, opcode, PC);
