...
int main() {
    ...
    fin.open("../XSet.txt",ios::in);                          //Use Boost techniques
    string s;
    while(fin>>s) {
        XSet.insert(s);
    }
    ...
    fin.open("../XSetAll.txt",ios::in);
    while(fin>>s) {
        XSetAll.insert(s);
    }
    fin.close();

    fin.open((string("../y")+sword).c_str(),ios::in);
    for(unsigned int c = 1;c<=DB_inv[sword].size();c++) {
        string s;
        fin>>s;
        BIGNUM *y = BN_new();
        ...
        REP(i,1,sz-1) {
            BIGNUM *tmp = BN_new();
            if(1 != BN_mod_exp(tmp, bxtoken[c-1][i-1], y, dhDB->p, ctxbn)) handleErrors();
            BIGNUM *subtr = BN_new();
            BIGNUM *one = BN_new();
            BN_dec2bn(&one, "1");
            ...
            cout<<"\n";
            BN_free(one);
            BIGNUM *exp2 = BN_mod_inverse(NULL, blinds[i-1], subtr, ctxbn);
            cout<<"Subtraction thing is\n";
            BN_print_fp(stdout,subtr);
            cout<<"\n";
            BN_print_fp(stdout,dhDB->p);
            cout<<"\n";
            BN_print_fp(stdout,blinds[i-1]);
            cout<<"\n";
            BN_print_fp(stdout,exp2);
            cout<<"\n";
            BIGNUM *tmp2 = BN_new();
            if(1 != BN_mod_exp(tmp2, tmp, exp2, dhDB->p, ctxbn)) handleErrors();
            string trytofind = string(BN_bn2hex(tmp2));
            cout<<"trytofind is "<<trytofind<<"\n";
            BN_free(exp2);
            BN_free(tmp);
            BN_free(tmp2);
        }
