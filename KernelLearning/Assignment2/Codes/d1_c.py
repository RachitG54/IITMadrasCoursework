...
for k in xrange(7):
	...
	plt.title('Dataset1, No of Classes:3,Model:1(c),degree:'+str(k))
	...
	plt.xticks(())
	plt.yticks(())
	plt.savefig("1(c)"+str(k)+".png", bbox_inches='tight')
	plt.close(fig)
	"""
...
plt.xlabel("degree")
plt.ylabel("accuracy")
plt.title('Accuracy for dataset1(c) on validation set')
plt.legend(loc='upper left')
plt.ylim([0,1.1])
plt.savefig("1(c)Models.png", bbox_inches='tight')
plt.close(fig)
