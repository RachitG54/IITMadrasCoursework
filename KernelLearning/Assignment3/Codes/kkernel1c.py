...
z = np.loadtxt("initial"+".txt", delimiter=',',dtype = np.int)
...
for iter in xrange(100):
	...
	if(iter == 0 or iter == 2 or iter == 5):
		...
		Z = np.loadtxt("kmeanskernel1(c)"+str(iter)+"-"+str(M)+".txt", delimiter=',',dtype = np.int)
		xr = xx.ravel()
		yr = yy.ravel()
		...
		plt.title('Dataset3, kernel k means clustering,iteration:'+str(iter)+'clusters = '+str(M))
		plt.legend(loc='best')
		...
		plt.xticks(())
		plt.yticks(())
		plt.savefig("kmeanskernel1(c)"+str(iter)+"-"+str(M)+".png", bbox_inches='tight')
		plt.close(fig)
