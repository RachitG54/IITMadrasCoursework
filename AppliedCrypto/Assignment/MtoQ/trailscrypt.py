...
#diffcryptanalysis
cols = 2 ** outputbits;
diffcrypt1 = np.zeros(shape=(rows,cols));
for x in xrange(rows):
	for z in xrange(rows):
		zstar = x^z;
		y = a11[z];
		ystar = a11[zstar];
		diffcrypt1[x][y^ystar] += 1;

print diffcrypt1
np.savetxt("diffcrypt1.csv", diffcrypt1, delimiter=",",fmt = "%d")


#diffcryptanalysis
cols = 2 ** outputbits;
diffcrypt2 = np.zeros(shape=(rows,cols));
for x in xrange(rows):
	for z in xrange(rows):
		zstar = x^z;
		y = a21[z];
		ystar = a21[zstar];
		diffcrypt2[x][y^ystar] += 1;

print diffcrypt2
np.savetxt("diffcrypt2.csv", diffcrypt2, delimiter=",",fmt = "%d")


#diffcryptanalysis
cols = 2 ** outputbits;
diffcrypt3 = np.zeros(shape=(rows,cols));
for x in xrange(rows):
	for z in xrange(rows):
		zstar = x^z;
		y = a31[z];
		ystar = a31[zstar];
		diffcrypt3[x][y^ystar] += 1;

print diffcrypt3
np.savetxt("diffcrypt3.csv", diffcrypt3, delimiter=",",fmt = "%d")

#diffcryptanalysis
cols = 2 ** outputbits;
diffcrypt4 = np.zeros(shape=(rows,cols));
for x in xrange(rows):
	for z in xrange(rows):
		zstar = x^z;
		y = a41[z];
		ystar = a41[zstar];
		diffcrypt4[x][y^ystar] += 1;

print diffcrypt4
np.savetxt("diffcrypt4.csv", diffcrypt4, delimiter=",",fmt = "%d")
...
diffcrypt1 = np.loadtxt("diffcrypt1.csv",delimiter = ",",dtype = np.int)
diffcrypt2 = np.loadtxt("diffcrypt2.csv",delimiter = ",",dtype = np.int)
diffcrypt3 = np.loadtxt("diffcrypt3.csv",delimiter = ",",dtype = np.int)
diffcrypt4 = np.loadtxt("diffcrypt4.csv",delimiter = ",",dtype = np.int)
